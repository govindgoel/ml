...
def get_available_gpus():
    ...
    if result.returncode != 0:
        raise RuntimeError(
            f"Error executing nvidia-smi: {result.stderr.decode('utf-8')}"
        )
...
def set_cuda_visible_device(gpu_index):
    ...
    print(
        f"Using GPU {gpu_index} with CUDA_VISIBLE_DEVICES={os.environ['CUDA_VISIBLE_DEVICES']}"
    )
...
def get_paths(
    base_dir: str,
    unique_model_description: str,
    model_save_path: str = "trained_model/model.pth",
):
    ...
    for i in tqdm(range(0, len(data_list), batch_size), desc="Normalizing x features"):
        ...
        for j, data in enumerate(batch):
            data.x[:, continuous_feat] = torch.tensor(
                batch_x_normalized[j * num_nodes : (j + 1) * num_nodes],
                dtype=data.x.dtype,
            )
...
def normalize_pos_features_batched(data_list, batch_size=1000):
    ...
    # Second pass: Transform the data
    for i in tqdm(
        range(0, len(data_list), batch_size), desc="Normalizing pos features"
    ):
        batch = data_list[i : i + batch_size]
        for data in batch:
            ...
            data.pos = torch.tensor(
                pos_normalized.reshape(num_nodes, 3, 2), dtype=data.pos.dtype
            )
...
def normalize_modestats_features_batched(data_list, batch_size=1000):
    ...
    for i in tqdm(range(0, len(data_list), batch_size), desc="Fitting scaler"):
        batch = data_list[i : i + batch_size]
        batch_modestats = np.vstack(
            [data.mode_stats.numpy().reshape(1, -1) for data in batch]
        )
        scaler.partial_fit(batch_modestats)

    # Second pass: Transform the data
    for i in tqdm(
        range(0, len(data_list), batch_size), desc="Normalizing modestats features"
    ):
        batch = data_list[i : i + batch_size]
        for data in batch:
            modestats_reshaped = data.mode_stats.numpy().reshape(1, -1)
            modestats_normalized = scaler.transform(modestats_reshaped)
            data.mode_stats = torch.tensor(
                modestats_normalized.reshape(6, 2), dtype=torch.float32
            )

    return data_list, scaler


def normalize_x_features_with_scaler(
    data_list, node_features, x_scaler, batch_size=100
):
    """
...
def one_hot_highway(datalist, idx):
    ...
    for data in datalist:
        ...
        data.x = torch.cat(
            (
                data.x[:, :idx],
                torch.tensor(one_hot, dtype=data.x.dtype),
                data.x[:, idx + 1 :],
            ),
            dim=1,
        )
...
def setup_wandb(args):
    ...
    wandb.init(
        ...
        name=args["unique_model_description"],
        config={
            k: v
            for k, v in args.items()
            if k not in ["project_name", "unique_model_description", "model_kwargs"]
        },
    )
    return wandb.config
...
def setup_wandb_metrics(predict_mode_stats=False):
    ...
    if predict_mode_stats:
        wandb.define_metric(
            "batch_train_loss-node_predictions", step_metric="batch_step"
        )
...
def create_gnn_model(
    gnn_arch: str, config: object, model_kwargs: dict, device: torch.device
):
    """
