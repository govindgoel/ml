...
districts_path = os.path.join(
    project_root, "data", "visualisation", "districts_paris.geojson"
)
...
def extend_geodataframe(
    gdf_base, gdf_to_extend, column_to_extend: str, new_column_name: str
):
    """
    ...
    if column_to_extend not in gdf_base.columns:
        raise ValueError(
            f"Column '{column_to_extend}' does not exist in the base GeoDataFrame"
        )
...
def calculate_avg_mode_stats(single_mode_stats_list: list):
    ...
    average_mode_stats.columns = [
        "mode",
        "avg_total_travel_time",
        "avg_total_routed_distance",
        "avg_trip_count",
    ]
...
def encode_modes(gdf):
    ...
    tensor_list = [
        torch.tensor(modes_encoded[col].values, dtype=torch.float)
        for col in modes_encoded.columns
    ]
    return tensor_list
...
def get_link_geometries(links_gdf_input):
    edge_midpoints = np.array(
        [
            (
                (geom.coords[0][0] + geom.coords[-1][0]) / 2,
    ...
    nodes = pd.concat(
        [links_gdf_input["from_node"], links_gdf_input["to_node"]]
    ).unique()
