...
result_path = os.path.join(
    project_root, "data", "train_data", "edge_features_with_net_flow_6000"
)
...
def process_result_dic(
    ...
    _, stacked_edge_geometries_tensor, edges_base, nodes = get_link_geometries(
        links_base_case
    )
    ...
    capacity_base_case = np.where(
        links_base_case["modes"].str.contains("car"), links_base_case["capacity"], 0
    )
    ...
    for key, df in tqdm(
        ...
        if isinstance(df, pd.DataFrame) and key != "base_network_no_policies":
            ...
            _, capacity_reduction, highway, freespeed = get_basic_edge_attributes(
                capacity_base_case, gdf
            )
...
def create_aggregated_edges_for_eign(links_gdf):
    ...
    # Convert positions
    stacked_positions = torch.tensor(aggregated_positions, dtype=torch.float)

    return edge_index, aggregated_features, edge_is_directed, stacked_positions
...
# process data to make it compatible with EIGN architecture
# EIGN input: edge_index (2 x E), edge_is_directed (E), x_signed (E x F_s), x_unsigned (E x F_u)
def process_result_dic_eign(
    ...
    edge_index, base_features, edge_is_directed_base, stacked_edge_geometries_tensor = (
        aggregated_data
    )
...
def validate_data(data: Data) -> bool:
    ...
    if data.y.shape != (data.num_nodes, 1):
        print(
            f"Invalid shape for data.y: expected ({data.num_nodes}, 1), got {data.y.shape}"
        )
        return False

    if data.edge_index.shape != (2, data.num_nodes):
        print(
            f"Invalid shape for data.edge_index: expected (2, {data.num_nodes}), got {data.edge_index.shape}"
        )
        return False

    if data.edge_is_directed.shape != (data.num_nodes,):
        print(
            f"Invalid shape for data.edge_is_directed: expected ({data.num_nodes},), got {data.edge_is_directed.shape}"
        )
        return False
    return True
