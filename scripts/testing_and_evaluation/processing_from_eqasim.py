...
def extract_last_digit(path):
    name = path.split('/')[-1]
    # Use regular expression to find the last digit in the string
    last_digit = re.findall(r'\d', name)[-1] if re.findall(r'\d', name) else None
    return int(last_digit) if last_digit is not None else None


def create_dic_seed_2_output_links(subdir: str):
    result_dic = {}
    for s in subdir:
        random_seed = extract_last_digit(s)
